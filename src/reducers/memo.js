...
export default function memo(state = initialState, action) {
  switch(action.type) {
    ...
    case types.MEMO_STAR_SUCCESS:
        // state.list.data.splice(action.index, 1, action.memo);
        // 특정 인덱스의 메모를 삭제하고, action.memo 로 대체 (해당필드만)
        // *splice 건 slice 나중에 합치기건 둘 다 가능
        let starBefore = state.list.data.slice(0, action.index);
        let starAfter = state.list.data.slice(action.index+1);
        // let newArr = [...before, action.memo ,...after];
        return {
            ...state,
          star: {
            ...state.star,
